...
def progressive_experiment(
    ...
    model_bs = bs.StanModel(model_lib=program_path, data=data, capture_stan_prints=False)
    rng = np.random.default_rng(seed)
...
def funnel_test():
    ...
    plot = (
            ...
            + pn.ggtitle(f"Bob's SAS sampler funnel test with {N} samples and seed {seed}")
    )
    plot.show()
    end_time = time.time()
