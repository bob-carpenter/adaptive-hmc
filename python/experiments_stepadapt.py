...
from algorithms import DRHMC_AdaptiveStepsize
import util
...
parser.add_argument('--nleapadapt', type=int, default=0, help='step size adaptation')
...
algfolder="stepadapt"
if args.nleapadapt == 0:
    savefolder = f"{savepath}/{algfolder}/nleap{args.nleap}/"   
else:
...
kernel = DRHMC_AdaptiveStepsize(D, lp, lp_g, mass_matrix=np.eye(D), distribution=args.dist, offset=args.offset, pbinom=args.pbinom)
